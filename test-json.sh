...
RESULT=`curl -s --cacert ./certs/participant1/intermediate/certs/ca-chain.cert.pem $CURL_CERT_PARAM \
  ...
  http://localhost:9001/v1/parties`
echo $RESULT
echo $RESULT | jq .
...
RESULT=`curl -s --cacert ./certs/participant1/intermediate/certs/ca-chain.cert.pem $CURL_CERT_PARAM \
  -X GET -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  http://$JSON_API_1_HOST:$JSON_API_1_PORT/v1/packages`
echo $RESULT | jq .
...
RESULT=`curl -s --cacert ./certs/participant1/intermediate/certs/ca-chain.cert.pem $CURL_CERT_PARAM \
  -X GET -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  http://$JSON_API_1_HOST:$JSON_API_1_PORT/v1/query`
echo $RESULT | jq .
...
RESULT=`curl -s --cacert ./certs/participant2/intermediate/certs/ca-chain.cert.pem  \
  ...
  http://$JSON_API_2_HOST:$JSON_API_2_PORT/v1/create`
...
RESULT=`curl -s --cacert ./certs/participant1/intermediate/certs/ca-chain.cert.pem  \
  -X POST -H 'Content-Type: application/json' \
  ...
  http://$JSON_API_1_HOST:$JSON_API_1_PORT/v1/exercise`
...
RESULT=`curl -s --cacert ./certs/participant2/intermediate/certs/ca-chain.cert.pem  \
  ...
  http://$JSON_API_2_HOST:$JSON_API_2_PORT/v1/exercise`
